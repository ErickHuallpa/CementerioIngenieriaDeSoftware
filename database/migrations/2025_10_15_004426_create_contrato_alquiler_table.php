...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('contrato_alquiler', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('id_nicho');
            $table->foreign('id_nicho')->references('id_nicho')->on('nicho')->onDelete('cascade');
            $table->date('fecha_inicio');
            $table->date('fecha_fin')->nullable();
            $table->integer('renovaciones')->default(0);
            $table->decimal('monto', 10, 2);
            $table->enum('estado', ['activo', 'vencido', 'renovado', 'cancelado'])->default('activo');
