...
<head>
    <meta charset="UTF-8">
    <title>Contrato Osario - {{ $difunto->persona->nombreCompleto ?? ($difunto->persona->nombre . ' ' . $difunto->persona->apellido) }}</title>
    <style>
...
<header>
    <div class="brand">
        <div style="width:58px;height:58px;border-radius:6px;background:linear-gradient(135deg,var(--primary),var(--secondary));display:flex;align-items:center;justify-content:center;color:white;font-weight:700;">
            CGP
        </div>
        <div>
...
<main>
    <h2 style="text-align:center;margin:0 0 6px 0;color:var(--secondary);font-size:15px;">CONTRATO DE ARRENDAMIENTO DE OSARIO</h2>
        ...
        <table class="info">
            <tr>
                ...
                <td>
                    ...
                    Dirección administrativa: Cementerio General de Potosí<br>
                </td>
            </tr>
            <tr>
    ...
    <section class="section">
        ...
        <p class="clausula">
            Por el presente contrato, EL ARRENDADOR otorga en uso al ARRENDATARIO el <strong>osario</strong> identificado como:
        </p>

        <table class="info">
            <tr><th>Pabellón</th><td>{{ $osario->pabellon->nombre ?? '—' }}</td></tr>
            <tr><th>Ubicación</th><td>Fila {{ $osario->fila ?? '—' }}, Columna {{ $osario->columna ?? '—' }}</td></tr>
            <tr><th>Tipo</th><td>Osario</td></tr>
        </table>
    </section>
    ...
    <section class="section">
        ...
        <table class="tabla-contrato">
            ...
            <tbody>
                <tr>
                    <td>Arriendo osario (plazo indicado)</td>
                    <td>Bs.</td>
                    <td style="text-align:right;">{{ number_format($contrato->monto ?? $osario->costo ?? 0, 2) }}</td>
                </tr>
            </tbody>
        </table>

        <div class="totales">TOTAL: Bs. {{ number_format($contrato->monto ?? $osario->costo ?? 0, 2) }}</div>
    ...
    <section class="section">
        ...
        <p class="clausula"><strong>Del Arrendador:</strong> Entregar el uso del osario en las condiciones descritas y permitir el acceso conforme al reglamento del cementerio.</p>
        <p class="clausula"><strong>Del Arrendatario:</strong> Pagar puntualmente las sumas convenidas, respetar el reglamento, mantener el osario en buen estado y notificar cualquier modificación en los datos personales.</p>
    </section>
